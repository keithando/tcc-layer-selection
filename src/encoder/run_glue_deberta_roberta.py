...
class ModelArguments:
    ...
    lora_alpha: int = field(
        default= 32,
        metadata={"help":"LoRA's Alpha"},
    )
    lora_r: int = field(
        default=16,
        metadata={"help":"LoRA's Rank"}
    )
    lora_dropout:float = field(
        default=0.1,
        metadata={"help":"LoRA's Dropout Rate"}
    )
...
def main():
    ...
    if model_args.lora_layers != None: 
        lora_config = LoraConfig(r = model_args.lora_r,
                                    lora_alpha = model_args.lora_alpha,
                                    lora_dropout = model_args.lora_dropout,
                                    task_type="SEQ_CLS",
                                    layers_to_transform=one_hot_encoded_to_layer_index_list(model_args.lora_layers)  
                                    )
    else:
        lora_config = LoraConfig(r = model_args.lora_r,
                                    lora_alpha = model_args.lora_alpha,
    ...
    # Hugging face peft model, including the lora modules that we specified, the method get_peft_model already freezes the original matrices.
    ...
    if training_args.do_train:
        ...
        # Merging LoRA into the model and saving it
        model = model.merge_and_unload()
        model.save_pretrained(f"{training_args.output_dir}/lora_{model_args.lora_layers}")
        tokenizer.save_pretrained(f"{training_args.output_dir}/lora_{model_args.lora_layers}")
